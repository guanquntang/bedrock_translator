...
def translate_file():
    ...
    logger.info(f"Starting batch translation of {filename} from {source_lang} to {target_lang}")
    ...
    try:
        ...
        translation_progress['total'] = len(lines)
        ...
        for i, line in enumerate(lines):
            if line:
                ...
                translation_progress['percent'] = int((i + 1) / len(lines) * 100)
        ...
        error_msg = str(e)
        flash(f'Batch translation error: {error_msg}', 'danger')
...
def call_bedrock_api(model_id: str, system_prompt: str, input_text: str) -> str:
    ...
    try:
        # 首先尝试使用invoke_model API，这是最可靠的方法
        ...
        if 'claude' in model_id.lower():
            ...
            if 'claude-3' in model_id.lower() or 'claude-3-5' in model_id.lower() or 'claude-3-7' in model_id.lower():
                # Claude 3/3.5/3.7 format
                body = json.dumps({
            ...
            # 如果不确定模型类型，尝试使用converse API作为备选
            try:
                logger.info("Model type unknown, trying converse API")
                response = bedrock_client.converse(
                    ...
                    inferenceConfig={
                        "temperature": 0.5,
                        "maxTokenCount": 2000  # 注意这里是maxTokenCount而不是maxTokens
                    }
                )
        ...
        error_msg = str(e)
        logger.error(f"Translation API error: {error_msg}", exc_info=True)
        
        # 如果是inference profile错误，尝试使用对应的基础模型
        if is_profile and "invalid" in error_msg.lower():
            try:
                # 从ARN中提取模型ID
                model_parts = model_id.split('/')
                if len(model_parts) > 1:
                    base_model_id = model_parts[-1]
                    logger.info(f"Trying with extracted base model ID: {base_model_id}")
                    return call_bedrock_api(base_model_id, system_prompt, input_text)
            except Exception as fallback_error:
                logger.error(f"Fallback to base model failed: {str(fallback_error)}")
